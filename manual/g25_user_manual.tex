 ...
 *     Its attributes are:
 *        - {\tt name}. The name of the general multivector type, for example {\tt mv}.
 *        - {\tt compress}. How to compress the multivector coordinates: {\tt byGrade} or {\tt byGroup} (explained in detail \ref mv_coord_sec "below").
 *        - {\tt coordinateOrder}. The order of coordinates: {\tt default} or {\tt custom} (explained in detail \ref mv_coord_sec "below").
 *        - {\tt memAlloc}. How to allocate memory for coordinates: {\tt full}, {\tt parityPure} or {\tt dynamic} (explained in detail \ref mv_coord_sec "below").
 ...
 *        - {\tt name}. The name of the specialized multivector type, for example {\tt vector}.
 *        - {\tt const} (optional). Can either be {\tt true} or {\tt false}. When true, the type is to be a constant type with no
 ...
 *        - {\tt type}. The {\tt type} attribute specified whether instances of the specifialized multivector class will carry
